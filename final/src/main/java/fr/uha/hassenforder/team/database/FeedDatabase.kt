...
class FeedDatabase (
    ...
    private suspend fun feedPersons(): LongArray {
        val dao: PersonDao = db.getPersonDao()
        val ids = LongArray(4)
    ...
    private suspend fun feedTeams(pids: LongArray) {
        val dao: TeamDao = db.getTeamDao()
        val team = getRandomTeam(pids[0])
        val tid = dao.upsert(team)
        dao.addTeamPerson(TeamPersonAssociation(tid, pids[0]))
        dao.addTeamPerson(TeamPersonAssociation(tid, pids[3]))
    }
    ...
    companion object {
        ...
        private fun getRandomFirstName(gender: Gender): String {
            ...
            return when (g) {
                Gender.BOY -> geRandomName(maleFirstNames)
                Gender.GIRL -> geRandomName(femaleFirstNames)
                else -> ""
            }
        }
